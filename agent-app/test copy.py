...
def selectFromTaos(ra):
  ...
  query =  "select  count(*) c from appiot.sensor_data2   {where} ;".format(where = where)
  #print(query);
...
#print(ra)
sorted_dates = sorted(ra, key=sort_by_date, reverse=True )#,  key=lambda x: int( x["add_on"]))
